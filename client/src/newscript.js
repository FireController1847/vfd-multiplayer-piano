...
$(function() {
  ...
  CanvasRenderer.prototype.resize = function(width, height) {
    ...
    this.keyMovement = Math.floor(this.whiteKeyHeight * 0.020);

    this.whiteBlipWidth = Math.floor(this.whiteKeyWidth * 0.75);
    this.whiteBlipHeight = Math.floor(this.whiteBlipWidth * 0.725);
    ...
    this.blackBlipWidth = Math.floor(this.blackKeyWidth * 0.65);
    this.blackBlipHeight = Math.floor(this.blackBlipWidth * 0.725);
    this.blackBlipY = Math.floor(this.blackKeyHeight - this.blackBlipHeight * 1.35);
    ...
    for (var j = 0; j < 2; j++) {
      ...
      for (var i in this.piano.keys) {
        ...
        if (key.sharp) {
          ...
          ctx.fillRect((this.whiteKeyWidth + this.xKeyOffset) * key.spatial + ctx.lineWidth,
            y + ctx.lineWidth,
            this.whiteKeyWidth - ctx.lineWidth * 2, this.whiteKeyHeight - ctx.lineWidth * 2);
        }
      }
    }
    ...
    for (var i in this.piano.keys) {
      ...
      var key = this.piano.keys[i];
      if (key.sharp) {
        key.rect = new Rect(this.blackKeyOffset + (this.whiteKeyWidth + this.xKeyOffset) * key.spatial, 0,
          (this.blackKeyWidth + this.xKeyOffset), this.blackKeyHeight);
      } else {
        key.rect = new Rect((this.whiteKeyWidth + this.xKeyOffset) * key.spatial, 0,
          (this.whiteKeyWidth + this.xKeyOffset), this.whiteKeyHeight);
      }
    }
  };
  ...
  CanvasRenderer.prototype.redraw = function() {
    ...
    for (var j = 0; j < 2; j++) {
      ...
      for (var i in this.piano.keys) {
        ...
        var x = Math.floor(key.sharp ? this.blackKeyOffset + (this.whiteKeyWidth + this.xKeyOffset) * key.spatial :
          (this.whiteKeyWidth + this.xKeyOffset) * key.spatial);
  ...
  // Background color
  (function() {
    var old_color1 = new Color("#206694");
    var old_color2 = new Color("#206694");
  ...
  var chat = (function() {
    ...
    return {
      ...
      send: function(message) {
        gClient.sendArray([{
          m: "a",
          message: message.replace(/[^\w\s`1234567890\-=~!@#$%^&*()_+,.\/<>?\[\]\\\{}|;':"]/g, '')
        }]);
      },
